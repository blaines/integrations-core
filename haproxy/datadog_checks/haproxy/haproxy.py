...
class HAProxyException(Exception):
    """
    Indicates an error when communicating with haproxy stats
    """
...
class HAProxy(AgentCheck):
    ...
    def check(self, instance):
        ...
        if tables:
            self._process_stick_table_metrics(tables, custom_tags=custom_tags)
    ...
    def _run_socket_commands(self, parsed_url, commands):
        ...
        if len(responses) != len(commands) + 1 or responses[len(responses) - 1] != '':
            raise HAProxyException("Got a different number of responses than expected")
    ...
    def _process_stick_table_metrics(
        ...
        for line in data:
            ...
            tags = ["haproxy_service:%s" % table.name, "type:%s" % table.type] + custom_tags
